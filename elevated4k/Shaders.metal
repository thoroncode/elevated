...
{
    float2 x = o - 0.5/1280;
    float4 g = t1.sample(s1, o);  // G-buffer world pos
    ...
    if (g.w > 0.5) {
        ...
        float t = length(g.xyz - u.q[4].xyz);
        float w = u.q[1].w - g.y;   // water level - surface.y  (< 0 = above water = terrain)

        if (w < 0) {
            // ── TERRAIN — exact m3 port ───────────────────────────────
            float3 n = cn(g.xz, 0.001*t, 12 - log2(t), u, t0);
            float  h = fbm(3*g.xz, 3, t0);
            float  r = no(666*g.xz, t0).x;
            ...
            // b(g, n, cn(g.xz,...)) — pass world position g
            c *= sl(g.xyz, n, cn(g.xz, 0.001*t, 5, u, t0), u, t0);

        } else {
            // ── WATER — exact m3 port ─────────────────────────────────
            // t=(q[1].w-q[4].y)/e.y; g=q[4]+e.xyzz*t
            t = (u.q[1].w - u.q[4].y) / e.y;
            g.xyz = u.q[4].xyz + e * t;   // world position of water hit
            float2 wXZ = g.xz;
