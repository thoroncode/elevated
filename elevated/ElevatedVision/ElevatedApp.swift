            ...
            CompositorLayer(configuration: ContentConfiguration()) { layerRenderer in
                Task { @MainActor in
                    ...
                    do {
                        ...
                        // (debug overlay, scene events) keep ticking.
...
/// Auto-opens the immersive space, dismisses the launch window, and pops the
/// debug overlay so screen captures include live state.
...
/// surfaced in the debug HUD. Diagnostic only — does not change the depth range
/// passed to the compositor.
nonisolated(unsafe) var capturedMinNearPlane: Float = 0
